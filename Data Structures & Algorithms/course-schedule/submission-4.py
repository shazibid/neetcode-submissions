class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        ...
        def dfs(crs):
            ...
            #only tracking the DFS path, so when we come back up, we can pop the course, if another class has this course as a prerec, it will auto True since we know that the path from this course is valid
            visited.remove(crs)
            courses[crs] = []
